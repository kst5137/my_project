...
def home():  # 함수명 수정 - 이름만 보고 접속되는 페이지를 확인할 수 있게!
    return render_template('template2(어빈님).html')
...
@app.route('/kakao_review', methods=['GET'])
def read_kakao_review():

    id = request.args.get('query')

    # for i in range(1) :
    data = requests.get(url='https://place.map.kakao.com/main/v/' + id).json()
